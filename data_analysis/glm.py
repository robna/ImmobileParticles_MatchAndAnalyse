...
def prepare_df4glm(wafer_results):
    ...
    # df4glm = df4glm.loc[df4glm.treatment != 'HCl']  # HCl is excluded from the model, because the aim is to best model the influence of n and image quality, and we know that the HCl wafer is strongly influenced by the treatment
...
def make_glm_arrays(df4glm):
    ...
    exogAll = sm.add_constant(exogAll, prepend=False)
...
def glm_residuals(fitted_model, endog, yAll, yhatAll):
    ...
    # TODO: rr was calculated before as yAll - yhatAll, now it is yAll - the linear regression between the original and the glm fitted values... which is correct?
    rr = yAll - meanRespAll  # residual response for all data points (= glm corrected rates, when using prediction mean)
    ...
    if settings.Config.glmPlots:
        ...
        # ax[0, 1].scatter(np.arange(1, len(yAll) + 1, len(y)), rr)
        ...
        ax[1, 1].bar(np.arange(1, len(yAll) + 1, 1), yhatAll, bottom=rr, color='C1')  #, width=0.2)
        ax[1, 1].scatter(np.arange(1, len(yAll) + 1, 1), np.maximum(rr, 0), color='black', zorder=4)
        ax[1, 1].set_ylim(0, 1)
        ax[1, 1].set_xlabel('id')
        ax[1, 1].set_ylabel('relative changes')
        ax[1, 1].set_title("\n".join(wrap('Relative changes: original (tops), attributed to n and image quality (upper bar) and attributed to treatment effects (bottom bar)', 80)))
        fig.show()
        plt.show(block=True)
...
def glm_from_df(wafer_results, formula='SUCCESS ~ pre_count + BDI'):
    ...
    glm_corrected_particle_losses, yhatAll, iv_uAll, iv_lAll = glm_residuals(glm_binom, endog)
    return glm_corrected_particle_losses, yhatAll, iv_uAll, iv_lAll, glm_binom
...
def count_loss_glm(wafer_results):
    ...
    if not settings.Config.manualBDI:
        glm_corrected_particle_losses_df['BDI'] = round(1 / (1 + np.exp((
                glm_binom.params[1] * wafer_results[settings.Config.glmImgQualiPredictors[0]] +\
                glm_binom.params[2] * wafer_results[settings.Config.glmImgQualiPredictors[1]]))), 3)
